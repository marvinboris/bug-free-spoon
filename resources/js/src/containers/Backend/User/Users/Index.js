...
import { getUsers, deleteUsers, resetUsers } from '../../../../store/actions/backend/users';
...
class Index extends Component {
    ...
    render() {
        const {
            content: {
                cms: {
                    pages: { components: { list: { action, see } }, backend: { pages: { users: { icon, title, add, index, form } } } }
                }
            },
        ...
        const content = <List array={data} loading={loading} data={JSON.stringify(users)} get={this.props.get} total={total} bordered add={add} link="/user/users/add" icon={icon} title={index} className="shadow-sm"
            fields={[
...
const mapDispatchToProps = dispatch => ({
    get: (page, show, search) => dispatch(getUsers(page, show, search)),
